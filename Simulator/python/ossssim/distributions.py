...
def merr(mag: numpy.ndarray, params: list = None):
    """
    merr as a function of magnitude given some parameters
    Args:
        mag:  the magnitude at which the uncertainty estimate is needed
        params:  list of 6 values describing the magnitude uncertainty function.

    The merr function is described in Petit et al. 2011.  Appropriate default values for CFEPS are given in the definitions module.
    """
    if params is None:
        params = definitions.MAG_ERR_PARAMS
    mag_th = mag
    ...
    A = numpy.sqrt(6.0) * (numpy.sqrt(2 * tmp) - 1)
    B = numpy.sqrt(6.0) * (1 - numpy.sqrt(2 * (1 - tmp)))
...
class Distributions:
    ...
    def __init__(self, seed=None, size=10**6):
        """
        Args:
            seed (None or int): the seed used to initialize the random number generator.
    ...
    def uniform(self, minimum: float = 0., maximum: float = 1.) -> numpy.ndarray:
        """
    ...
    def normal(self, mu, sigma):
        ...
        return self.rnd_gen.normal(mu, sigma, self.size)
    ...
    def truncated_sin_normal(self, mu, sigma, minimum, maximum):
        ...
        Args:
            ...
            maximum (float): the maximum value of the probability density to be created, in radians.
        Returns:
    ...
    def power_knee_divot(self, alpha_bright, h_max, h_min=1., h_break=None, alpha_faint=None, contrast_ratio=1.):
        ...
        Shankman, C., Gladman, B.~J., Kaib, N., et al. 2013, ApJL, 764, L2. doi:10.1088/2041-8205/764/1/L2
