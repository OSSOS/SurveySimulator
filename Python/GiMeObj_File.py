...
class Obj:
    ...
    def draw_file(self):
        ...
        This distribution represents a sample distribution reading from a file.

        When reading from a file the set of values received is duplicated until they have reached the size given at
        object instantiation. The size of arrays produced will be greater than or equal to this instantiation size.

        There is often a relatively small number of values read in from a file. looping over these few values,
        even when all other variables are randomly determined, can lead to unexpected results. It is important that
        when the survey simulator requires more values than those provided by the supplied text files,
        that modifications are made to these source values to generate a wider distribution. Otherwise, unintended
        statistical effects will appear in the survey simulator results.

        The exact procedure to be used in generating these additional values is left to the user, and their specific
        requirements.

        This specific example draws from the classified test particles from the Brasser and Morbidelli 2013 planetary
        migration simulation.  The particles included occupy the region from 10-155 AU and were classified in Pike
        et. al 2016.

        If you use this simulation output, please cite the original simulation and the classification paper:

        Brasser, R., & Morbidelli, A. 2013, Icarus, 225, 40
        Pike, R. E., Lawler, S., Brasser, R., Shankman, C. J., Alexandersen, M., and
            Kavelaars, J. J.,  submitted to AJ  2016
        """

        self.comments = "file"

        # Reads from a text file the name of a different text file filled with values to be read into a distribution
        # array.
        with open("model.txt", 'r') as f:
            model_file = f.read()
            model_file = model_file.strip()  # Strips the newline character from the end of the model name.

        # When reading in variables from a file these variables must be read in first in order to determine the
        # length of all generated arrays to follow. If different variable arrays have different lengths,
        # indexing over the set of arrays will not behave as expected.
        self.a = self.read_file(model_file, 0)  # AU
        self.e = self.read_file(model_file, 1)
        self.i = np.deg2rad(self.read_file(model_file, 2))  # Radians

        self.node = self.read_file(model_file, 3)
        self.peri = self.read_file(model_file, 4)
        self.m = self.read_file(model_file, 5)
        ...
        self.blur_e("e", 0.01)  # eccentricity requires special consideration for values above 1.0

        # When first generating "blurred" populations, the use of the diagnostic .plot() function is recommended. This
        # allows the user to verify that the generated distributions behave as expected.
        # e.g. uncomment the following line:
        # self.plot("a", "e", "i")
        ...
        self.resamp = self.constant(np.deg2rad(0))  # units in radians.
