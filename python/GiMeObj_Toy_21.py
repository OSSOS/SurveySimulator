...
class Obj:
    ...
    def draw_symm(self):
        ...
        This distribution represents a toy model of objects in the 2:1 symmetric resonance island with Neptune.
        """
        ...
        self.i = self.constant(np.deg2rad(0.001))  # units in radians
    ...
    def draw_asymm_lead(self):
        ...
        This distribution represents a toy model of objects in the 2:1 asymmetric leading resonance island with Neptune.
        """
        ...
        # choose value of phi sinusoidally from resamp
        phi = np.deg2rad(80) + np.sin(self.uniform(0, 2*np.pi))*self.resamp
        lambda_n = np.deg2rad(333.5078)  # Neptune's mean longitude on 1 Jan 2013
    ...
    def draw_asymm_trail(self):
        ...
        This distribution represents a toy model of objects in the 2:1 asymmetric trailing resonance island with
        Neptune.
        """
        ...
        # choose value of phi sinusoidally from resamp
        phi = np.deg2rad(280) + np.sin(self.uniform(0, 2*np.pi))*self.resamp
        lambda_n = np.deg2rad(333.5078)  # Neptune's mean longitude on 1 Jan 2013
...
def initialize():
    ...
    obj1 = Obj(size)
    obj1.draw_symm()  # Symmetric resonance
    ...
    # obj1.plot("peri", "resamp")
    # obj2.plot("peri", "resamp")
    # obj3.plot("peri", "resamp")
...
def gimeobj():
    ...
    selection = rnd_gen.uniform()

    # Randomly select a source instance to draw an object from, then return that object to driver.

    if selection < 1/3:  # Symmetric
    ...
    elif selection < 2/3:  # Asymmetric leading
